...
class MutualInformationEstimator():
    def __init__(self, model, device, samples, noise_distribution, data_distribution, input_dataset):
        # samples: (N,)
        ...
        # TODO: For the time being, we are assuming that the hidden dimension is 1 and that
        # the input dataset are scalar values
    ...
    def compute_mutual_information(self, num_samples_per_outcome):
        uncond_entropy = self.compute_unconditional_entropy()

        mutual_information = uncond_entropy
        for i in range(self.input_dataset.shape[0])
            curr_outcome = self.input_dataset[i]
            cond_entropy = self.data_distribution(curr_outcome) * self.compute_conditional_entropy(curr_outcome)
            mutual_information -= cond_entropy

        return mutual_information

    def compute_conditional_entropy(self, sample):
        # Compute outputs for curr_outcome from different noise realizations
        inputs = np.tile(curr_outcome, (num_samples_per_outcome,1))
        model_outputs = self.model(torch.from_numpy(inputs).float().to(device))
        ...
        integrand = lambda x: estimated_conditional_distribution(x) * np.log(1./estimated_conditional_distribution(x))

        # SP estimator: h(p_{T_\ell | X = x_i}) \approx h(\hat{p}_{S_{\ell}^{(i)}} \ast \phi)
        conditional_entropy = integrate.quad(integrad, -np.inf, np.inf)

        return conditional_entropy

    def compute_unconditional_entropy(self):
        ...
        estimated_distribution = lambda x: (1./self.num_samples)*np.sum(self.noise_distribution(x - self.samples))

        # Integrand for computing the entropy. i.e. f(x) log (1/f(x))
        integrand = lambda x: estimated_distribution(x) * np.log(1./estimated_distribution(x))

        # SP estimator: h(p_{T_\ell}) \approx h(\hat{p}_{S_\ell} \ast \phi)
        unconditional_entropy = integrate.quad(integrand, -np.inf, np.inf)
